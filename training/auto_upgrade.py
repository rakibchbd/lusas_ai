...
from lusas_ai.model_registry import MODEL_IDS, foundation_config, stable_path
from lusas_ai.scorecard import record as record_scorecard
from lusas_ai.skills import ensure_builtin, record_use
from lusas_ai.upgrade_log import current_count, format_version, record
...
def _run_once_impl(root: Path) -> dict:
    ...
    model_loop_enabled = (
        ...
        and any(foundation_config(settings, model_id)["model"] or foundation_config(settings, model_id)["path"] for model_id in MODEL_IDS)
    )
    code_loop_enabled = (
    ...
    if model_loop_enabled:
        if inputs_unchanged:
            ...
            report = evaluate(model_id, candidate, eval_path, settings=settings, max_new_tokens=128)
            progress(
            ...
            baseline_report = None
            if stable_path(settings, model_id).is_dir():
                progress("evaluating stable baseline")
                baseline_report = evaluate(
                    model_id,
                    stable_path(settings, model_id),
                    eval_path,
